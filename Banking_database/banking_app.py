...
def login(conn, card_numbers, pin_numbers):
    """logging requires card number and card pin-LISTS VERSION"""
    # for debugging
    print()
...
# sql version of login
def login_sql(conn, card_numbers, pin_numbers):
    """logging requires card number and card pin-SQL Version"""
    # for debugging
    print()
    card_log = input("Enter your card number:\n")
    pin_log = input("Enter your PIN:\n")
    print(banking_database.check_if_card_exists(conn, card_log))
    if banking_database.check_if_card_exists(conn, card_log)[0] and pin_log == banking_database.check_if_card_exists(conn, card_log)[1]:
        print("\nYou have successfully logged in!\n")
        after_login(conn, card_log, card_numbers, pin_numbers)
    else:
        print("Wrong card number or PIN!")
...
def after_login(conn, card_log, card_numbers, pin_numbers):
    ...
    while game:
        ...
        if choices == '1':
            ...
            elif account_to_transfer[-1] != luhn_check(account_to_transfer):
    ...
    2. if the card number starting with 4?
       if the card number a valid card number, passing the luhn's check?
            ...
            if transfer_amount > balance:
                ...
                print("Success")
        """
    if account_to_transfer in banking_database.get_all_card_numbers(conn):
...
def card_list(card_numbers, pin_numbers):
    """adds the created card to their respective lists"""
...
def menu(conn, card_numbers, pin_numbers):
    ...
    while site_in:
        ...
        if create == '1':
            ...
            login_sql(conn, card_numbers, pin_numbers)
        elif create == '0':
            # print(card_numbers)
            # print(pin_numbers)
            # try:
